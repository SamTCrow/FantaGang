<script lang="ts" setup>
	...
	import { propsDef } from "v-calendar/dist/types/src/use/calendar.js";
		...
		const { data: partite, refresh: fetchPartite } = await useFetch<Partita[] | []>(
			() => {
		...
		const clear = () => {
			...
			(state.legaId = legaSelect.value?.id ?? 1), (state.squadraCasa = undefined);
		...
		const options = computed(() => {
			if (!squadreInLega.value || !partite.value) return [];
			const squadre = squadreInLega.value?.map((squadra) => ({
				id: squadra.id,
				nome: squadra.nome,
				disabled: partite.value?.some(
					(giocate) => giocate.squadraCasa === squadra.id || giocate.squadraOspite === squadra.id
				),
			}));
			return squadre;
		});
